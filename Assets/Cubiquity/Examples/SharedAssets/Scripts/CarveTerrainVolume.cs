...
public class CarveTerrainVolume : MonoBehaviour
{
    ...
    private void ClickMouseLeftButton()
    {
        ...
        if (Input.GetMouseButton(0))
        {
            if (!isMouseAlreadyDown)
            {
                ...
                if (hit)
                {
                    ...
                    //CreateVoxels((int)pickResult.volumeSpacePos.x, (int)pickResult.volumeSpacePos.y, (int)pickResult.volumeSpacePos.z, range);
                }
    ...
    public void CreateVoxels(int xPos, int yPos, int zPos, int range)
    {
        // Initialise outside the loop, but we'll use it later.
        int rangeSquared = range * range;
        ...
        for (int z = zPos - range; z < zPos + range; z++)
        {
            for (int y = yPos - range; y < yPos + range; y++)
            {
                for (int x = xPos - range; x < xPos + range; x++)
                {
                    ...
                    if (distSquared < rangeSquared)
                    {
                        terrainVolume.data.SetVoxel(x, y, z, newMaterialSet);
                    }
                }
            }
        }
